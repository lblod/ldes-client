...
- **STARTUP_DELAY**: a delay in milliseconds to introduce before any LDES fetching is done. Allows to wait for other services such as the database. Default: 10000 
...
- **SKOLEMIZE_BLANK_NODES**: boolean indicating whether or not blank nodes should be skolemized before being inserted in the triplestore. Default: false
- **SKOLEMIZATION_BASE_URI**: the base URI to use when skolemizing blank nodes. Default: http://mu.semte.ch/bnode/
